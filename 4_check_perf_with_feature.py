...
#%%
model_fn = '/home/lithtp/sanofi/bfactor/lightning_logs/version_9/best.ckpt'
...
for batch in tqdm.tqdm(dl_val):
        src, tgt, mask, lens, feat = batch['c_res_id'].to(model.device), \
            batch['y'].to(model.device), \
            batch['mask'].to(model.device), batch['L'], \
            batch['feat'].to(model.device)
        N = len(lens)
        lens = lens.view(-1,)
        src = src.transpose(0,1).contiguous()
        keep_ind = (~mask).to(torch.float)
        feat = feat.transpose(0,1).contiguous()
        output = model(src, src_key_padding_mask = mask, feat=feat).transpose(0,1)[:,:,0]
